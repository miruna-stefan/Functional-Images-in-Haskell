...
fuseTransformations = foldr combineIdenticalTransformations []
  where
    {- daca gasim 2 transformari de tip Translation consecutive, le combinam
    intr-o singura transformare Translation cu coordonatele x si y adunate -}
    combineIdenticalTransformations (Translation x1 y1) (Translation x2 y2 : restOfTransformations)
        = Translation (x1 + x2) (y1 + y2) : restOfTransformations
    {- daca gasim 2 transformari de tip Scaling consecutive, le combinam
    intr-o singura transformare Scaling cu coordonatele x si y inmultite -}
    combineIdenticalTransformations (Scaling x1) (Scaling x2 : restOfTransformations)
        = Scaling (x1 * x2) : restOfTransformations
    {- daca transformarea curenta nu poate fi combinata cu urmatoarea, o lasam 
    singura -}
    combineIdenticalTransformations t restOfTransformations = t : restOfTransformations
