...
{- helper care se apeleaza recursiv, crescand astfel nivelul de indentare pe masura ce avansam in AST-}
printLevel :: Int -> RegionAST -> String
printLevel level (R regionShape) = 
    let indent = replicate (2 * level) ' ' -- indent reprezinta cat de adanca este indentarea pentru nivelul curent
...
{- pentru a rezolva ultimul TODO, am modificat functia auxiliara printLevel, care nu era compozitionala,
intr-o functie compozitionala, pe care am rescris-o direct ca pe o reducere,
folosind functionala foldRegionAST-}

printLevel' :: RegionAST -> (Int -> String)
printLevel' = foldRegionAST f
  where
    f :: RegionCombiner (Int -> String)
    f regionShape = \level ->
      let indent = replicate (2 * level) ' '
      in case regionShape of
        FromPoints points -> indent ++ "FromPoints " ++ show points
        Rectangle width height -> indent ++ "Rectangle " ++ show width ++ " " ++ show height
        Circle radius -> indent ++ "Circle " ++ show radius
...
{- instantierea clasei Show pentru RegionAST, folosind functia auxiliara printLevel' -}
instance Show RegionAST where
    show region = printLevel' region 0
...
toTransformation = foldTransformationAST f
  where
    f :: TransformationCombiner Transformation
    f (Translation tx ty) = S.translation tx ty
    f (Scaling s) = S.scaling s
    f (Combine transformations) = S.combineTransformations transformations
...
basicTransformationCount = foldTransformationAST f
  where
    f :: TransformationCombiner Int
    f (Translation _ _) = 1
    f (Scaling _) = 1
    f (Combine transformations) = foldr (+) 0 transformations
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea lenesa presupune faptul ca expresiile nu sunt calculate fix in momentul
    in care sunt intalnite, ci abia atunci cand ne trebuie rezultatul lor. In cazul de fata, 
    daca dorim, de exemplu, sa accesam doar prima componenta a perechii (numarul de regiuni),
    partea de calcul legata de a doua componenta a perechii (numarul de transformari) nu va fi
    efectuata (daca nu este accesata explicit). Astfel, evaluarea lenesa ne ajuta sa
    evitam calculul inutil al unor expresii, care nu sunt necesare pentru rezultatul final.
...
basicEntityCount = foldRegionAST f
  where
    f :: RegionCombiner (Int, Int)
    f (FromPoints _) = (1, 0)
    f (Rectangle _ _) = (1, 0)
    f (Circle _) = (1, 0)
    f (Complement (r1, t1)) = (r1, t1)
    f (Union (r1, t1) (r2, t2)) = (r1 + r2, t1 + t2)
    f (Intersection (r1, t1) (r2, t2)) = (r1 + r2, t1 + t2)
    f (Transform transf (r1, t2)) = (r1, t2 + transfCount)
      where
        transfCount = basicTransformationCount transf
